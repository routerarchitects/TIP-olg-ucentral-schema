	...
	"$defs": {
		...
		"globals": {
			...
			"properties": {
				...
				"dhcp-defaults": {
					"description": "Site-wide fallback values for DHCP server configuration. Any interface with an ipv4.dhcp block that omits one of these fields inherits the value from here. Renderers must resolve these fallbacks against the raw configuration, not the schemareader output, because the per-interface ipv4.dhcp.lease-time carries its own default that the reader injects when omitted. The resolution order for a given interface is therefore the per-interface value, then this site-wide default, then the per-interface schema default. No default is set here so an omitted site-wide value stays genuinely absent rather than masking the per-interface default.",
					"type": "object",
					"properties": {
						"lease-time": {
			...
			"allOf": [
				...
				},
				{
					...
					"properties": {
						...
						"web-root-url": {
							"description": "A URL where the webroot should be downloaded from.",
							"type": "string"
						},
						"web-root-checksum": {
							"description": "The SHA256 of the file located at web-root-url.",
							"type": "string"
						},
						"idle-timeout": {
		...
		"interface": {
			...
			"properties": {
				...
				"captive": {
					"description": "Name of the captive portal profile (from services.captive-profiles) applied to unauthenticated clients on this interface. If omitted, this interface has no captive portal. Has no effect if no matching profile is present in services.captive-profiles.\n",
					"type": "string",
					"examples": [
						"guest-public"
					]
		...
		"service.captive-profile": {
			"description": "A named captive portal profile. Bound to one or more interfaces through the interface \"captive\" field. Carries the same configuration as services.captive plus a name used as the reference key.",
			"allOf": [
				{
					"$ref": "#/$defs/service.captive"
				},
				{
					...
					"properties": {
						"name": {
							"description": "Unique name of this captive profile, referenced by an interface's \"captive\" field.",
							"type": "string",
							"examples": [
								"guest-public",
								"guest-staff"
		...
		"service.rrm.chanutil": {
			"description": "RRM policy based on Channel Utilization for optimization.",
			"type": "object",
			"properties": {
				"interval": {
					"description": "The interval to check channel utilization (in seconds).",
					"type": "number",
					"minimum": 240
				},
				"threshold": {
					"description": "The airtime utilization threshold.",
					"type": "number",
					"minimum": 0,
					"maximum": 99,
					"examples": [
				...
				"consecutive-threshold-breach": {
					"description": "The number of times the Channel Utilization is higher than the threshold before triggering channel optimization.",
					"type": "integer",
					"minimum": 1
				},
				"algo": {
					"description": "The algorithm for channel optimization.",
					"type": "string",
					"examples": [
						"rcs",
						"acs"
		...
		"service": {
			...
			"properties": {
				...
				"captive-profiles": {
					"description": "A list of named captive portal profiles that interfaces may select via their \"captive\" field. Independent of the device-wide services.captive object.",
					"type": "array",
					"items": {
						"$ref": "#/$defs/service.captive-profile"
					}
				},
				"gps": {
			...
			"properties": {
				...
				"translation": {
					...
					"properties": {
						"address": {
							"type": "string",
							"description": "Translated source address (IP  keyword like \"masquerade\")."
						},
						"port": {
							"type": "string",
							"description": "Translated port  (e.g. \"80\")."
						},
						"options": {
							"type": "object",
							"additionalProperties": false,
		...
		"nat": {
			...
			"properties": {
				"snat": {
					"type": "object",
					"description": "IPV4 Source NAT Configurations (SNAT).",
					"additionalProperties": false,
					"properties": {
						"rules": {
							"type": "array",
				...
				"dnat": {
					"type": "object",
					"description": "IPV4 Destination NAT Configurations (DNAT).",
					"additionalProperties": false,
					"properties": {
						"rules": {
							"type": "array",
